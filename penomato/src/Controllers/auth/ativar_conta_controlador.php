...
// ============================================================
// Buscar categoria do usuário para definir se ativa imediatamente
$usuario_dados = buscarUm(
    "SELECT categoria FROM usuarios WHERE id = :id LIMIT 1",
    [':id' => $registro['usuario_id']]
);
$ativa_imediato = ($usuario_dados && $usuario_dados['categoria'] === 'colaborador');

iniciarTransacao();
try {
    $campos_atualizar = ['status_verificacao' => 'verificado'];
    if ($ativa_imediato) {
        $campos_atualizar['ativo'] = 1;
    }
...
if ($ativa_imediato) {
    ...
    $_SESSION['mensagem_sucesso'] = "E-mail confirmado! Sua conta está aguardando aprovação do gestor.";
}
header('Location: ' . APP_BASE . '/src/Views/auth/login.php');
exit;
